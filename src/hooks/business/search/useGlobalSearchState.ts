...
export interface UseGlobalSearchStateReturn {
  ...
  handleSuggestionSelect: (suggestion: string, setQuery: (query: string) => void) => void;
...
export const useGlobalSearchState = ({ 
  ...
  const handleQueryChange = useCallback((query: string, setQuery: (query: string) => void) => {
    setQuery(query);
    setShowResults(query.trim().length >= minQueryLength);
  }, [minQueryLength]);

  const handleSuggestionSelect = useCallback((suggestion: string, setQuery: (query: string) => void) => {
    setQuery(suggestion);
    setShowResults(true);
  }, []);
