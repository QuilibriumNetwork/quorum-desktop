...
export function useMessageFormatting(options: UseMessageFormattingOptions) {
  ...
  const handleImageClick = useCallback((e: React.MouseEvent<HTMLImageElement>, imageUrl: string) => {
  ...
  const isMentioned = useCallback((userAddress: string) => {
  ...
  const processTextToken = useCallback((token: string, messageId: string, lineIndex: number, tokenIndex: number) => {
    // Check for mentions
