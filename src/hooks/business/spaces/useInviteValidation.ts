...
export const useInviteValidation = () => {
  const [validatedSpace, setValidatedSpace] = useState<ValidatedSpace | undefined>(undefined);
  const [validationError, setValidationError] = useState<string>();
  const [isValidating, setIsValidating] = useState<boolean>(false);
  const { apiClient } = useQuorumApiClient();

  const parseInviteLink = useCallback((inviteLink: string): InviteInfo | null => {
    const validPrefixes = [
    ...
    const matchingPrefix = validPrefixes.find(prefix => inviteLink.startsWith(prefix));
    if (!matchingPrefix) {
      return null;
    }
  ...
  const validateInvite = useCallback(async (inviteLink: string) => {
