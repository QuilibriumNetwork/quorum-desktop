...
if (!toPlayground && !fromPlayground && !interactive && !syncNewer) {
  console.error(
    `${colors.red}Error: You must specify a sync direction${colors.reset}`
  );
  console.error(
    'Use --to-playground, --from-playground, --sync-newer, or --interactive'
  );
  console.error(
    `${colors.dim}Optional flags: --force-lingui (sync Lingui-equivalent files), --dry-run, --no-backup${colors.reset}`
  );
  process.exit(1);
}
...
if ([toPlayground, fromPlayground, syncNewer].filter(Boolean).length > 1) {
  console.error(
    `${colors.red}Error: Cannot use multiple direction flags together${colors.reset}`
  );
  console.error(
    'Choose one: --to-playground, --from-playground, or --sync-newer'
  );
  process.exit(1);
}
...
if (!all && components.length === 0 && !interactive) {
  console.error(
    `${colors.red}Error: Specify components to sync or use --all${colors.reset}`
  );
  process.exit(1);
}
  ...
  try {
    ...
    toDelete.forEach((backup) => {
      try {
        ...
        console.log(
          `    ${colors.dim}Cleaned up old backup: ${backup.name}${colors.reset}`
        );
      } catch (error) {
...
function copyFile(source, destination) {
  ...
  if (!dryRun) {
    ...
    if (backupPath) {
      console.log(
        `  ${colors.dim}Created backup: ${path.basename(backupPath)}${colors.reset}`
      );
    }
...
function normalizeLinguiContent(content) {
  ...
  normalized = normalized.replace(
    /\/\/ Playground-specific: Skip Lingui for demo purposes\s*\n\/\/ Real mobile app will use full Lingui integration\s*\n?/g,
    ''
  );
...
function syncFile(sourcePath, destPath, direction) {
  if (fs.existsSync(sourcePath)) {
    console.log(
      `  ${colors.cyan}→${colors.reset} ${path.basename(sourcePath)}`
    );
...
async function syncComponent(componentName, direction) {
  ...
  const playgroundComponentPath = path.join(
    MOBILE_PLAYGROUND_PATH,
    componentName
  );

  // For sync-newer mode, we need to check each file individually
  if (direction === 'sync-newer') {
    return await syncComponentByNewerFiles(
      componentName,
      mainComponentPath,
  ...
  const sourcePath =
    direction === 'to-playground' ? mainComponentPath : playgroundComponentPath;
  const destPath =
    direction === 'to-playground' ? playgroundComponentPath : mainComponentPath;

  if (!fs.existsSync(sourcePath)) {
    console.log(
      `${colors.yellow}⚠${colors.reset} Component '${componentName}' not found in source location`
    );
    return false;
  }
  ...
  const files = fs
    ...
    .filter(
      (file) =>
        fileExtensions.some((ext) => file.endsWith(ext)) || file === 'types.ts'
    );
  ...
  for (const file of files) {
    ...
    if (isRoutingFile(file) && !force) {
      console.log(
        `  ${colors.dim}Skipping ${file} (architectural difference - use --force to override)${colors.reset}`
      );
      continue;
    }
    ...
    if (interactive) {
      const sourceLabel =
        direction === 'to-playground' ? 'main app' : 'playground';
      const destLabel =
        direction === 'to-playground' ? 'playground' : 'main app';
...
async function syncComponentByNewerFiles(
  componentName,
  mainComponentPath,
  playgroundComponentPath
) {
  ...
  if (fs.existsSync(playgroundComponentPath)) {
    const playgroundFiles = fs
      .readdirSync(playgroundComponentPath)
      .filter(
        (file) =>
          fileExtensions.some((ext) => file.endsWith(ext)) ||
          file === 'types.ts' ||
  ...
  if (allFiles.size === 0) {
    console.log(
      `${colors.yellow}⚠${colors.reset} No files found for component '${componentName}'`
    );
    return false;
  }
  ...
  for (const file of allFiles) {
    ...
    if (comparison === 'identical') {
      console.log(
        `  ${colors.green}✓${colors.reset} ${file} ${colors.dim}(identical)${colors.reset}`
      );
      continue;
    }
    ...
    if (comparison === 'lingui-equivalent' && ignoreLingui) {
      console.log(
        `  ${colors.green}≈${colors.reset} ${file} ${colors.dim}(Lingui-equivalent, skipped)${colors.reset}`
      );
      continue;
    }
    ...
    if (comparison === 'missing') {
      console.log(
        `  ${colors.dim}Skipping ${file} (file not found in either location)${colors.reset}`
      );
      continue;
    }
    ...
    const statusSuffix =
      comparison === 'lingui-equivalent' ? ' (Lingui-equivalent)' : '';

    if (newerLocation === 'main') {
      console.log(
        `  ${colors.cyan}→${colors.reset} ${file} ${colors.dim}(main app → playground)${statusSuffix}${colors.reset}`
      );
      ...
      console.log(
        `  ${colors.cyan}←${colors.reset} ${file} ${colors.dim}(playground → main app)${statusSuffix}${colors.reset}`
      );
  ...
  console.log(
    `  ${colors.green}✓ Synced ${syncedCount} files${colors.reset} ${colors.dim}(${mainToPlayground}→ playground, ${playgroundToMain}← main)${colors.reset}`
  );
  return syncedCount > 0;
}
...
async function main() {
  ...
  if (!force && hasUncommittedChanges()) {
    console.log(
      `${colors.yellow}⚠ Warning: You have uncommitted changes${colors.reset}`
    );
  ...
  console.log(
    `${colors.dim}Note: Web playground imports directly from main app (no sync needed)${colors.reset}`
  );
  ...
  if (all) {
    const mainPrimitives = new Set(
      getPrimitiveDirectories(MAIN_PRIMITIVES_PATH)
    );
    const playgroundPrimitives = new Set(
      getPrimitiveDirectories(MOBILE_PLAYGROUND_PATH)
    );
    componentsToSync = [
      ...new Set([...mainPrimitives, ...playgroundPrimitives]),
    ];
  }
  ...
  for (const component of componentsToSync) {
    ...
    if (interactive && direction === 'interactive') {
      ...
      if (!syncDirection) {
        console.log(
          `${colors.dim}Invalid option, skipping ${component}${colors.reset}`
        );
        continue;
      }
    }
  ...
  console.log(
    `${colors.green}Components synced:${colors.reset} ${totalSynced}`
  );

  if (ignoreLingui) {
    console.log(
      `${colors.dim}Lingui awareness: Enabled (Lingui-equivalent files skipped by default)${colors.reset}`
    );
  } else {
    console.log(
      `${colors.dim}Lingui awareness: Disabled (--force-lingui used)${colors.reset}`
    );
  }
