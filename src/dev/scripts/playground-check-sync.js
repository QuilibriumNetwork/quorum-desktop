...
function checkComponentSync(componentName) {
  ...
  if (fs.existsSync(mainComponentPath)) {
    ...
    mainFiles.forEach((file) => {
      ...
      const comparison = compareFilesLinguiAware(
        mainFilePath,
        playgroundFilePath
      );
      ...
      const fileResult = {
        ...
        inSync:
          comparison === 'identical' || comparison === 'lingui-equivalent',
        comparison: comparison, // New field to track comparison status
        mainMtime,
        playgroundMtime,
        mainSize: mainChecksum ? fs.statSync(mainFilePath).size : null,
        playgroundSize: playgroundChecksum
          ? fs.statSync(playgroundFilePath).size
          : null,
      };
...
function main() {
  ...
  allPrimitives.forEach((primitive) => {
    ...
    const hasLinguiEquivalent = syncStatus.files.some(
      (file) => file.comparison === 'lingui-equivalent'
    );

    // Check if all files are either identical or lingui-equivalent
    const allFilesInSyncOrLinguiEquivalent =
      syncStatus.files.length > 0 &&
      syncStatus.files.every(
        (file) =>
          file.comparison === 'identical' ||
          file.comparison === 'lingui-equivalent'
      );
    ...
    if (syncStatus.inSync && syncStatus.files.length > 0) {
      ...
      if (hasLinguiEquivalent) {
        console.log(
          `${colors.green}≈${colors.reset} ${primitive} ${colors.dim}(Lingui-equivalent)${colors.reset}`
        );
      } else {
        console.log(
          `${colors.green}✓${colors.reset} ${primitive} ${colors.dim}(in sync)${colors.reset}`
        );
      }
      ...
      console.log(
        `${colors.red}✗${colors.reset} ${primitive} ${colors.yellow}(out of sync)${colors.reset}`
      );
      ...
      syncStatus.files.forEach((file) => {
        if (!file.inSync) {
          ...
          if (file.mainExists && file.playgroundExists) {
            console.log(
              `     Main:       ${formatDate(file.mainMtime)} (${formatBytes(file.mainSize)})`
            );
            console.log(
              `     Playground: ${formatDate(file.playgroundMtime)} (${formatBytes(file.playgroundSize)})`
            );
          }
        }
      });
  ...
  console.log(
    `${colors.green}In sync:${colors.reset} ${totalInSync} components`
  );
  console.log(
    `${colors.red}Out of sync:${colors.reset} ${totalOutOfSync} components`
  );
  console.log(
    `${colors.dim}Lingui awareness: Enabled (Lingui-equivalent files treated as in-sync)${colors.reset}`
  );

  if (totalOutOfSync > 0) {
    console.log(`\n${colors.yellow}💡 To sync components, run:${colors.reset}`);
    console.log(
      `   yarn playground:sync --sync-newer --all     ${colors.dim}# Auto-sync based on newer files (recommended)${colors.reset}`
    );
    console.log(
      `   yarn playground:sync --to-playground --all  ${colors.dim}# Copy all from main app to playground${colors.reset}`
    );
    console.log(
      `   yarn playground:sync --from-playground --all ${colors.dim}# Copy all from playground to main app${colors.reset}`
    );
    console.log(
      `   yarn playground:sync --interactive          ${colors.dim}# Choose direction for each component${colors.reset}`
    );
    console.log(
      `   yarn playground:sync --force-lingui --all   ${colors.dim}# Force sync even Lingui-equivalent files${colors.reset}`
    );
  }
