...
import { Button, Input, Icon, Tooltip, Spacer, Callout, ScrollContainer } from '../../primitives';
...
const ConversationList: React.FunctionComponent<ConversationListProps> = ({
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent) => {
    // Enter picks the top-of-list match — useful when the user has typed
    // enough to disambiguate to a single conversation.
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-2 px-2 py-1.5 bg-[var(--color-field-bg)] hover:bg-[var(--color-field-bg-focus)] focus-within:bg-[var(--color-field-bg-focus)] focus-within:ring-1 focus-within:ring-[var(--color-field-border-focus)] rounded-lg mb-2 transition-colors">
        <Icon name="search" size="sm" className="text-subtle flex-shrink-0" />
        <Input
          value={searchInput}
      ...
      <ScrollContainer height="xs" className="!border-subtle bg-[var(--color-field-bg)]">
        {isEmpty ? (
          ...
          filteredOptions.map((option) => {
            ...
            const hasValidAvatar =
              option.avatar && !option.avatar.includes(DefaultImages.UNKNOWN_USER);

            // Selected row: pin both the resting AND mouse-down background
            // to the sidebar's active accent token, so press-down doesn't
            // briefly revert to the hover colour. Matches the SpacesSidebar
            // active-row recipe for visual consistency.
            const rowClasses = isSelected
              ? 'bg-[var(--color-bg-sidebar-active-accent)] active:bg-[var(--color-bg-sidebar-active-accent)]'
              : 'hover:bg-[var(--color-field-option-hover)] active:bg-[var(--color-bg-sidebar-active-accent)]';

            return (
              <div
                key={option.value}
                className={`flex items-center justify-between p-2 cursor-pointer transition-colors min-w-0 ${rowClasses}`}
              ...
              >
                ...
                {isSelected && (
                  <Icon
                    name="check"
                    size="sm"
                    className="text-main flex-shrink-0 ml-2"
                  />
                )}
              </div>
            );
