  ...
  async requestSync(spaceId: string): Promise<void> {
    try {
      this.enqueueOutbound(async () => {
        ...
        const inboxKey = await this.messageDB.getSpaceKey(spaceId, 'inbox');
        const configKey = await this.messageDB.getSpaceKey(spaceId, 'config');
        logger.log('[SyncService.requestSync] configKey exists:', !!configKey, 'publicKey length:', configKey?.publicKey?.length, 'prefix:', configKey?.publicKey?.substring(0, 16), 'privateKey length:', configKey?.privateKey?.length, 'privKey prefix:', configKey?.privateKey?.substring(0, 16));
        if (configKey) {
          const pubBytes = hexToSpreadArray(configKey.publicKey);
          const privBytes = hexToSpreadArray(configKey.privateKey);
          logger.log('[SyncService.requestSync] pubBytes first 8:', pubBytes.slice(0, 8), 'privBytes first 8:', privBytes.slice(0, 8));
        }
