...
- [MASTER RECAP: control-message authorization](issues/.open/2026-06-25-MASTER-RECAP-control-message-auth.md) — **no longer only release timing.** The desktop space fix is now measured as shipped (`prod-2026-07-31`), but the mobile half is unverifiable from the repo (no git tags), and the table keeps gaining rows: two open follow-ups held privately, plus DONE rows for the 2026-08-20 verify/resolve work and for #366, which gave the space sync frames a membership-based authorization rule rather than a bare signature check. Closing it needs someone with mobile release visibility.
