...
6g. ⭐⭐ **ROUND Y (2026-07-30) — exact replication, both desktops live this time: 17/20 on both, missing the identical [5, 11, 17], reload never recovers, zero warnings.** Loss rate stable at 3/20 across consecutive rounds; **common-mode loss is now established across three independent rounds (U, X, Y)**. ⛔ It also **REFUTES round X's slow-send lead** — 19 of 20 sends were slow this round and the losses were unremarkable in timing. Do not re-derive it. ⭐ New and separate: **receipt acks are lost on the mobile leg** — desktop A holds receipts for all 17 landed messages while mobile A (same account) holds only 4, so B emitted them and one of A's own devices never got them; corroborates field round 26's 0/10 read-acks vs 11/11 posts. Live hypothesis is now **ratchet chain position**, testable in one rig round (`git debug` rebases the rig onto master, which now carries the burst button, giving DH epoch per sent message). See `docs/transport-measurements.md` § ROUND Y.
...
| `quorum-desktop/.agents/tasks/2026-07-30-keepalive-fix-session-handoff.md` | ⭐ **START HERE to implement the fix.** Research brief (keepalive best practice, mobile battery, send-durability patterns), the two-layer fix, and the falsifiable acceptance test (81 drops → ≈0; burst 20/20) |
| `quorum-desktop/.agents/bugs/2026-07-30-mobile-frames-lost-into-a-dying-websocket.md` | ⭐⭐⭐⭐ **THE ROOT CAUSE.** No keepalive anywhere in the transport; the connection is reaped every ~16 s with close code 1006, and frames written in the blind window before the client notices are lost silently and never retried. 81 drops in 25 idle minutes; desktop shows the same cycle. Read this before any further DM-loss work |
...
| ⭐⭐⭐ | **THE TOP ITEM — fix the dying socket.** Layer 1: add an application-level keepalive to the shared transport (none exists on any platform), after establishing what actually reaps the connection at ~16 s (relay/proxy idle timeout vs unanswered server ping — may be an infra fix, not a client one). Layer 2: stop treating `ws.send()` as delivery — widen the replay window so frames written before an undetected break are retried. Verify each with a burst round + the DM doctor | `bugs/2026-07-30-mobile-frames-lost-into-a-dying-websocket.md` §6 | shared + both clients (+ infra) |
