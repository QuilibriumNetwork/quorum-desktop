...
| | **THE TOP ITEM — ship the send-retention fix to `quorum-shared`.** Measured and validated on a real device across five rounds: baseline 16-17/20, and **20/20 with a 12 s retention window**, `duplicates: 0` throughout. The existing `pendingEnvelopes` rescue already does the right thing and is simply ~1 s wide while the blind window is 3.5-5 s. **PR only — no version bump, no publish** (lead dev's call). The brief covers whether this is a keeper (the mechanism yes, the wall-clock constant no) and the full lead-dev ask | `tasks/2026-07-31-ship-send-retention-to-quorum-shared.md` | shared |
