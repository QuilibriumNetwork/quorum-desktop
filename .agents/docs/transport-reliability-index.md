...
6c. **A third client-side loss path was found live on 2026-07-29 and is only partly mitigated**: a session replacement mints a new receiving inbox and deletes the old state, so frames the peer already addressed to the old inbox arrive with no state and are never persisted — **366 distinct messages in one capture**. They are *stranded, not destroyed* (the cleanup named the wrong mailbox and silently removed nothing, now corrected to retain deliberately), so the data is probably recoverable, but the messages still do not arrive. Desktop only. See `bugs/2026-07-29-session-replacement-strands-in-flight-frames.md` — read its §7 first, the original write-up was wrong on the central point.
7. Remaining client-side work is not orphaning the inbox on session replacement (6c), send-side durability on mobile spaces, receipt truthfulness runtime verification, and hygiene items (ghost devices, junk state rows).
...
| `quorum-desktop/.agents/bugs/2026-07-29-session-replacement-strands-in-flight-frames.md` | ⚠️ **OPEN, partially mitigated.** A session replacement orphans the receiving inbox; frames already addressed to it arrive with no state and are never persisted — **366 distinct messages in one live capture** on the operator's desktop. ⚠️ **Read §7 first**: this file's original claim (permanent destruction) was WRONG and was corrected after adversarial review. The delete named the device inbox while the frame sat in a session inbox, so it removed nothing — the frames are stranded, probably still recoverable. Guardrail shipped; the real fix is unbuilt and v1 was refuted. Mobile unaffected (it keeps a persistent per-conversation inbox keypair) |
...
| ⚠️ | **PARTIAL 2026-07-29 — frames stranded on an orphaned inbox.** A session replacement mints a new receiving inbox and deletes the old state; frames the peer already addressed to the old one arrive with no state and are not persisted. Captured live on the operator's desktop: **366 distinct messages** in one window. The guardrail shipped (the branch no longer attempts a delete that named the wrong mailbox and silently removed nothing), so the frames are stranded rather than lost — **but they are still not delivered.** The real fix, not orphaning the inbox, is unbuilt: v1 was refuted in review because the replace path selects rows by tag alone and would sweep any retained row | `bugs/2026-07-29-session-replacement-strands-in-flight-frames.md` | desktop |
...
| 8 | **Measure the NATURAL session-replacement rate**, without a harness driving the account. The 36 replacements in the 07-29 capture were mostly harness-induced, so the frequency — and therefore how much the stranding bug actually costs users — is unmeasured | `bugs/2026-07-29-session-replacement-strands-in-flight-frames.md` §4 | desktop |
