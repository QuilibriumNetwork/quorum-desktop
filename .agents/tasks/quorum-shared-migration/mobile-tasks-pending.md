...
| 2026-06-08 | [`2026-06-08-mobile-converge-bio-length-to-shared.md`](file:///D:/GitHub/Quilibrium/quorum-mobile/.agents/tasks/quorum-shared-migration/2026-06-08-mobile-converge-bio-length-to-shared.md) | Converge all three mobile bio editors (`profile-setup` 160, `UnifiedProfileEditModal` 256, `SpaceSettingsModal` 280) to consume shared `MAX_BIO_LENGTH=160` + `validateUserBio`. Removes inconsistency + adds XSS check. Desktop already uses shared (160) everywhere. User-visible cap reduction for any existing 161–280 char bios on mobile. | [quorum-desktop `feat/port-per-space-bio`](https://github.com/QuilibriumNetwork/quorum-desktop/pulls) (pending — port surfaced the drift) | ✅ required | 📋 open |
