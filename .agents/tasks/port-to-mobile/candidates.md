...
| 31 | **Channel icon picker — feature parity** (mobile has it but: 20 icons vs desktop's 49; NO outline/filled variant toggle; raw-hex vs named-color storage) | LOW-MED | icon-SET + color-palette are NOT shared (desktop-local); promoting them is the real fix (ADDITIVE) | 🚧 task-dropped (mobile 7.2 + shared task) |
