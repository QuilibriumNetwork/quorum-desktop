...
| 1 | **#29** | **Non-owner read-only access to public invite URL** | small UI + read existing `space.inviteUrl` | ✅ live on mobile | **Smallest port in the table.** `space.inviteUrl` is already replicated to every member via the encrypted space manifest. Desktop hides it for non-owners because the only consumer (SpaceSettings > Invites tab) is owner-gated. The port is pure UI exposure — a lightweight read-only modal: show URL, copy, share. **No generate/regenerate controls for non-owners** (would error). No shared-promotion needed. |
...
### #29 Non-owner read-only access to the existing public invite URL 🟢 ready to pick
