...
|---|---|---|---|---|---|
| **1** | **#1** | **Discover spaces** | ~85 hook + ~280 screen | ✅ | **User's first pick (2026-06-01).** Capability-verified missing on desktop: `JoinSpaceModal` + `AddSpaceModal` are invite-link-only. No public directory, no category filtering, no server-curated browse. `useSpaceJoining` exists and is reusable for the join mutation; we just need the discovery UI on top. Hook is tiny (~85 LOC), most of the work is the screen (~280 LOC). |
| **2** | **#6** | **Public profile UI** | ~150 svc + ~130 hooks + new UI | ✅ | **Capability-verified missing on desktop.** Desktop has rich profile infrastructure (`UserProfile.tsx`, `UserProfileEdit.tsx`, `useUserProfileModal`, `useUserProfileActions`, `useSpaceProfile`, `useProfileImage`) but all of it is space-membership-scoped. No `isProfilePublic` toggle in edit, no public-profile fetch flow for users you don't share a space with, no fallback resolver. Closes a known shared-migration loose end (shared has the `isProfilePublic` + `farcasterLink` field since 2026-05-28; desktop has zero consumer). Same Ed448 signing swap as Reporting. |
| — | **#5** | **Reporting** | — | ⏸️ deprioritized | **Not a priority on desktop right now** (user call 2026-06-01). Capability still missing on desktop; revisit when trust/safety becomes a product priority. See `### #5` notes. |
...
**Recommended first port: #1 Discover spaces** (user's pick, 2026-06-01). Both top picks have been capability-verified missing on desktop. #1 has a smaller surface area (no crypto-signing swap, no shared-package fields to consume — just an HTTP GET + a join action already wired up via `useSpaceJoining`), making it a cleaner first port. #6 stays as the queued follow-up.
...
- **2026-06-01** — **Capability-verified missing on desktop.** Desktop has rich profile infrastructure: [`components/user/UserProfile.tsx`](../../../src/components/user/UserProfile.tsx) (viewing), [`UserProfileEdit.tsx`](../../../src/components/user/UserProfileEdit.tsx) (editing own), [`hooks/business/ui/useUserProfileModal.ts`](../../../src/hooks/business/ui/useUserProfileModal.ts), [`hooks/business/user/useUserProfileActions.ts`](../../../src/hooks/business/user/useUserProfileActions.ts), [`hooks/business/spaces/useSpaceProfile.ts`](../../../src/hooks/business/spaces/useSpaceProfile.ts), [`hooks/business/user/useProfileImage.ts`](../../../src/hooks/business/user/useProfileImage.ts) — but all space-membership-scoped. No `isProfilePublic` toggle in edit, no public-profile fetch endpoint usage, no fallback resolver for users you don't share a space with.
...
- **Mobile sources:** [`services/profile/publicProfile.ts`](../../../../quorum-mobile/services/profile/publicProfile.ts) (publish/unpublish, ~125 LOC), [`hooks/useUserPublicProfile.ts`](../../../../quorum-mobile/hooks/useUserPublicProfile.ts) (fetch, ~40 LOC), [`hooks/useMembersWithPublicProfileFallback.ts`](../../../../quorum-mobile/hooks/useMembersWithPublicProfileFallback.ts) (fallback resolver, ~130 LOC), used in `ProfileModal.tsx` + DM screen.
...
- Queued as #2 in the ranking. Pick up after #1 ships.
