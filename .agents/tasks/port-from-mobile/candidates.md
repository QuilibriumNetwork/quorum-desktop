...
| **6** | **Public profile** (opt-in publish + resolve-by-address; backfills chat rendering for users we don't share a space with) | `services/profile/publicProfile.ts`, `hooks/useUserPublicProfile.ts`, `hooks/useMembersWithPublicProfileFallback.ts`, consumed in `UnifiedProfileEditModal.tsx` (toggle), `dm/[id].tsx` (header), `SpaceChatArea.tsx` + `DMChatArea.tsx` (fallback) | B | 🚧 | **In progress 2026-06-08** ([task file](2026-06-08-port-public-profile.md)). Closes the `UserConfig.isProfilePublic` shared-migration loose end (PR #159, 2026-05-28). **Capability is narrower than original framing** — there is no directory or full-text search on mobile either; server exposes only `GET /users/:addr/public-profile` (no enumeration endpoint). The speculative People tab on desktop (`/discover/people`) is removed in the same PR. |
...
| **2** | **#6** | **Public profile UI** | ~125 svc + ~170 hooks + new UI | ✅ | **Capability-verified missing on desktop.** Desktop has rich profile infrastructure (`UserProfile.tsx`, `UserProfileEdit.tsx`, `useUserProfileModal`, `useUserProfileActions`, `useSpaceProfile`, `useProfileImage`) but all of it is space-membership-scoped. No `isProfilePublic` toggle in edit, no resolve-by-address fetch, no fallback resolver. Closes a known shared-migration loose end (shared has the `isProfilePublic` + `farcasterLink` field since 2026-05-28; desktop has zero consumer). 🚧 in progress 2026-06-08. **Scope is narrower than original framing**: mobile has no directory/search either — the server only exposes resolve-by-known-address. Bundles removal of speculative `/discover/people` People tab in the same PR. |
...
**🚧 #6 Public profile UI in progress (2026-06-08, branch `session-2026-06-08`).** Scope clarified during pickup: mobile has no profile directory either (server doesn't expose user enumeration — see [#6 notes](#6-public-profile-ui--in-progress-2026-06-08)), so the port is the resolve-by-address plumbing mobile actually ships (toggle, DM-header backfill, member fallback resolver). The speculative `/discover/people` People tab is being removed in the same PR. See [task file](2026-06-08-port-public-profile.md).
...
### #6 Public profile UI 🚧 in progress (2026-06-08)
