...
| **18** | ~~**Image attachments / media library save**~~ | `services/media/imageAttachment.ts`, `services/media/saveToLibrary.ts`, `services/media/customAssets.ts` | E | ❌ | **Already on desktop.** Mobile's `imageAttachment.ts` + `customAssets.ts` both say "Mirrors/Matches desktop behavior" in their headers — they were ports FROM desktop. Desktop equivalents: `src/utils/imageProcessing/{unifiedProcessor,compressor,config}.ts` + `src/hooks/business/ui/useCustomAssets.ts`. Constants match exactly on the message-attachment pipeline (1200×1200, q0.8, 300 px thumbnail threshold, 25 MB input cap, 2 MB GIF cap). Emoji/sticker dimensions drift by design (desktop 36 px emoji / 400 px sticker source = correct for desktop's fixed CSS render sizes; mobile 128 px / 512 px = correct for variable DPR). `saveToLibrary` is mobile-OS-specific — Electron context menu's "Save Image As" handles the equivalent for free. One minor real drift: mobile iterates compression to ≤1 MB for E2EE; desktop does a single q0.8 pass and assumes 1200×1200 lands in range. Not enough to keep the row open. See [`### #18`](#18-image-attachments--media-library-save-removed-desktop-has-it) notes. |
...
### #18 Image attachments / media library save ❌ removed (desktop has it)

- **2026-06-09** — **Decision: removed from candidates (Class E).** Verified after `quorum-shared-migration` re-baselining. The original row was a hedge ("A/E ❔ — could yield small UX-parity wins"). The investigation closes it cleanly:
  - **Mobile's headers admit it.** `services/media/imageAttachment.ts` line 5: "Mirrors desktop behavior". `services/media/customAssets.ts` line 5: "Matches desktop behavior. Images are automatically resized…". These are mobile ports OF the desktop pipeline, not capability gaps on desktop.
  - **Desktop equivalents already exist:** `src/utils/imageProcessing/unifiedProcessor.ts` + `compressor.ts` + `config.ts` (the centralized config-driven engine; `unifiedProcessor` replaced 6 specialized processor files), `src/hooks/business/ui/useCustomAssets.ts` (emoji + sticker dropzones), `src/components/modals/ImageModal.tsx` (lightbox), and the `useFileUpload` / `useSpaceFileUploads` / `useWebFileUpload` upload paths.
  - **`saveToLibrary` is mobile-OS specific.** Saving to the device Photos library is a mobile concept (`expo-media-library`). On Electron, the browser/chrome context menu's "Save Image As…" is the equivalent affordance, available for free on every `<img>`/lightbox surface. No port needed.
- **Constant-diff result** (done as a spot-check, since that's all the row was reducing to):
  - **Message-attachment pipeline matches exactly**: maxWidth 1200, maxHeight 1200, quality 0.8, thumbnail threshold 300px, thumbnail max 300px, input cap 25MB, GIF hard cap 2MB.
  - **Emoji dimension is different by design**: mobile 128px source vs desktop 36px source. Desktop renders emoji at a fixed 24x24px CSS box and 36×36 is the correct 1.5× retina source. Mobile renders at variable RN sizes (24px in body, larger in picker/reactions) and needs the bigger source.
  - **Sticker dimension is different by design**: mobile 512px source vs desktop 400×600. Desktop displays at `gifMaxDisplayWidth: 300`, so 400px source is enough; mobile needs more headroom for DPR scaling.
  - **One real but minor drift**: mobile iterates compression to ≤1MB (the E2EE storage target) via `targetFileSizeBytes: 1024 * 1024`; desktop does a single q0.8 pass and assumes 1200×1200 will land in range. Edge case for very large high-detail photos. Not enough to keep the row open; would be a 20-line follow-up if anyone ever reports oversized attachments.
- **Lesson** (consistent with the rule captured during #3 Reply tracking): a mobile service file that says "mirrors desktop" in its header is a signal to capability-grep desktop first, before adding the row at all. Future inventory passes should fast-path that check.
...
*Last updated: 2026-06-09 — **#18 Image attachments / media library save ruled out as Class E.** Capability-verification pass + constant diff confirmed: mobile's `imageAttachment.ts` and `customAssets.ts` are documented in their own headers as ports OF desktop behavior; desktop equivalents (`src/utils/imageProcessing/{unifiedProcessor,compressor,config}.ts` + `useCustomAssets`) match exactly on the message-attachment pipeline (1200×1200, q0.8, 300px thumbnail threshold, 25MB input cap, 2MB GIF cap). Emoji (36 vs 128) and sticker (400 vs 512) dimensions drift by design — desktop renders at fixed CSS sizes, mobile at DPR-scaled sizes. `saveToLibrary` is mobile-OS-specific (Electron context menu handles it). One minor real drift: mobile iterates to ≤1MB for E2EE; desktop does a single pass. Not enough to keep open. Row updated in main table + new `### #18` notes block added. Lesson: when a mobile service header says "mirrors desktop behavior", fast-path the capability-grep on desktop before adding the row.*
