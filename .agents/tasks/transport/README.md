...
status: living — ⚠️ THIS FILE CARRIES STATUS ON PURPOSE. It is the one exception to the rule in the index. If a row here disagrees with the doc it points at, the doc wins and this row is stale — fix it.
created: 2026-08-01
...
> then tasks/transport/index.md §2 (the read-first ladder).
> Never quote a figure without checking tasks/transport/measurements.md.
> ```
...
| # | what | size | doc |
|---|---|---|---|
| B1 | **Desktop has no send retention at all** — it doesn't consume the shared client that got the fix. Preferred route: adopt the shared `BrowserWebSocketClient` (already carries the fix, but has no production consumer yet) | small | `2026-08-01-desktop-send-retention-gap.md` |
