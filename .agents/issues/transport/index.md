...
> ## 👉 Looking for "what is open right now"? That is NOT this file.
>
> **Read [`transport-reliability-STATUS.md`](README.md) first.**
> One screen, every open item, ranked by who owns it (blocked upstream / owed a device
> round / ready to build), plus the list of docs here that are living reference and will
> **never** be "done". Added 2026-08-01 because this file had grown into an archive and
> nobody could tell what was still live.
>
> **This file remains the MAP and the ARCHIVE** — every doc, PR and issue, plus the record
> of how the reasoning moved and what got retracted. That is worth having. It is just not a
> front door.
...
> then this file's read-first ladder (§2) before forming a theory. It indexes every
> transport/DM doc across quorum-desktop, quorum-mobile and quorum-shared.
> ```

⚠️ **This file does not carry status.** Statuses move daily and duplicating them
here would rot. Each row points at the doc that owns it. The exceptions are §2
(the ladder, which is about reading order), §8 (statuses verified against git and
found stale in their own files), and the STATUS file above, which carries status
**on purpose** and accepts the maintenance cost in exchange for being answerable.
...
| # | read | why |
|---|---|---|
| 0 | **`quorum-desktop/.agents/tasks/transport/README.md`** | ⭐ **what is open, on one screen.** Start here. Also tells you which docs in this cluster are living reference and will never be "done" |
| 1 | **this file** | the map + the archive |
| 1b | `quorum-desktop/.agents/tasks/transport/measurements.md` | **the numbers.** Every run, its configuration, its result, and what it changed — append-only. Read it before quoting any figure, and add to it after any run |
...
| 3 | `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md` | the *DM ratchet* entry point: root cause, the ten dead hypotheses (§3), the offline tools (§5) |
...
| 5 | `quorum-desktop/.agents/tasks/transport/dm-ratchet-upstream-divergences.md` | the 8 shipped divergences from the upstream DR implementation, and why each exists |
...
**→ Every run and every number lives in [`measurements.md`](measurements.md).**
...
[`issues/.done/2026-07-28-dm-receive-holds-ratchet-lock-across-http.md`](../.done/2026-07-28-dm-receive-holds-ratchet-lock-across-http.md) §5-RESULT.
...
| `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md` | **DM ratchet entry point.** Root cause, 10 dead hypotheses, offline tools, the rig |
| `quorum-desktop/.agents/tasks/transport/runbook.md` | **THE RUNBOOK for operator-assisted test rounds.** A fresh session asked to run a manual DM round follows this file: the shipped tools (mobile burst button PR #200, desktop `/dev/dm-doctor` PR #275), the 6 rules, the test estate + burned letters, the step-by-step protocol, the queued experiments |
...
| `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-captures.md` | **archive only** — round data + findings A→AL + retracted mechanisms. Cited by letter from the entry point |
...
| `quorum-desktop/.agents/tasks/transport/2026-07-17-dm-dead-session-autoheal.md` | detect a dead session via missing receipts and repair without a manual reset |
...
| `quorum-desktop/.agents/tasks/transport/2026-08-01-desktop-send-retention-gap.md` | **desktop consumes NONE of the send-retention fix** — it has its own WebSocket implementation and no retention, no replay, not even mobile's old ~1 s mid-batch requeue. Verified in code 2026-08-01. Low urgency (Chromium pongs in ms, so the blind window rarely opens) but the protection is *missing*, not unnecessary |
| `quorum-desktop/.agents/tasks/transport/2026-07-17-dm-dead-session-autoheal.md` | **re-scoped 2026-08-01, priority INVERTED.** Heal action 1 (single-loss resend) is subsumed by send retention; heal action 2 (dead-direction re-init) is uncovered, cannot be covered by retention, and now has two proven causes (#183 item 2b, stale-returning-device). Gated on the receipt-truthfulness runtime check |
...
| `quorum-desktop/.agents/tasks/transport/2026-08-01-desktop-ack-self-echo-guard.md` | desktop lacks mobile's `senderId !== self` guard on ack intercepts — fan-out confirmed, damage unconfirmed |
...
| `quorum-desktop/.agents/tasks/transport/2026-07-29-stale-returning-device-dm-sends-vanish-and-misfile.md` | a device returning with months-stale state loses 100% of sends toward the peer silently (orphaned session inboxes, #273 seen from the SENDER side) and its self-sync copies persist under a ghost self-conversation. Demonstrated live, probe-verified |
| `quorum-desktop/.agents/tasks/transport/2026-07-29-transport-debug-workflow-and-tooling.md` | the manual-round workflow (3 rules) + tool suite T1-T4 (resident DM doctor, mobile burst button, NATIVE socket probe, round runner) + the ordered experiment queue |
...
| `quorum-desktop/.agents/tasks/transport/dm-ratchet-upstream-divergences.md` | the 8 shipped divergences from upstream DR + the two open upstream questions. Lead-dev facing |
...
- **Desktop:** §6 of `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md`. The headless harness (§4-F) has replaced most of the need for it.
...
| 5 | **Port the #265 stale-bucket mitigation to mobile** — desktop-only today | `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md` §5-B1′ | mobile |
| 5b | **Desktop has NO send retention** — it does not consume the shared client that got the fix. Verified in code; not previously tracked. Small change, removes a failure class instead of mitigating one. Preferred route is adopting the shared `BrowserWebSocketClient`, which already carries the fix but has no production consumer | `quorum-desktop/.agents/tasks/transport/2026-08-01-desktop-send-retention-gap.md` | desktop |
| 5c | **DM dead-session auto-heal, re-scoped 2026-08-01** — build heal action 2 (dead-direction re-init) only; heal action 1 is subsumed by send retention. Two proven causes now sit behind it (#183 item 2b; the stale-returning-device bug). ⚠️ Gated on item 3 below: the detector consumes delivery receipts, which have been unstable across four rounds | `quorum-desktop/.agents/tasks/transport/2026-07-17-dm-dead-session-autoheal.md` | both |
...
| 6 | **Desktop ack self-echo guard** — desktop has no `senderId !== self` on its ack intercepts where mobile does; the own-device fan-out is confirmed real, whether it does damage is not | `quorum-desktop/.agents/tasks/transport/2026-08-01-desktop-ack-self-echo-guard.md` | desktop |
| 7 | **What supplies the out-of-order delivery that forms the stale bucket in the field?** — the one remaining live question on the crate bug | `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md` §5-D | — |
...
| doc | was | now |
|---|---|---|
| `quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md` | "the live work item is reviewing and merging branch `feat/dm-stale-bucket-retry`" | that branch **merged** as PR #265, 2026-07-27. **Bug stays OPEN** — the cause is upstream, the mitigation is desktop-only, §5-D unanswered |
...
**Crypto / ratchet work:**
```
quorum-desktop/.agents/tasks/transport/index.md                     (map — read first)
quorum-desktop/.agents/tasks/transport/2026-07-26-dm-desktop-to-desktop-resurfaced.md     (§3 dead ends, §5 tools)
quorum-desktop/.agents/tasks/transport/dm-ratchet-upstream-divergences.md
...
quorum-desktop/.agents/tasks/transport/2026-08-01-desktop-ack-self-echo-guard.md
...
**New measurements go in `measurements.md`, not here.** That file is
