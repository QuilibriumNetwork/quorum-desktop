...
| **OPEN** | **PRIVATE:** desktop `.agents/issues/.secret/` (`2026-08-20-sender-authentication-is-opt-in-mechanism-level-fix-design.md`) | **The mechanism-level design this recap called for at lines 108-115, now written up.** Diagnoses why per-type hardening keeps leaving gaps (the type list is opt-IN, so a new feature inherits nothing and nothing fails), and proposes compiler-enforced classification in `quorum-shared` plus one structural correction that is a prerequisite for the row above. 7 slices, each with its own rollout-safety note. **Needs a named owner for slices 3-7** — see §7 of that file, which argues this proposal has now been made twice and shelved once. **Execution queue is now public: [`receive-auth-hardening/`](../receive-auth-hardening/README.md).** |
...
| **OPEN** | [`receive-auth-hardening/`](../receive-auth-hardening/README.md) (public) | **The execution queue for the durability work**, one brief per PR, created 2026-08-21 so this effort stops being invisible to every index and progress view. Five public briefs; three more held privately until the fixes above reach users. **Start with the identifier-mismatch audit** — it is read-only and its result changes what the rest is worth. Records a SECOND failure mode the design above does not cover: a handler that is correctly enrolled and correctly verified, but authorizes a different object than the one it acts on. Both 2026-08-21 defects were of that shape. |
