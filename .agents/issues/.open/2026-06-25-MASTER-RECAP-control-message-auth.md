...
| **DONE** | quorum-desktop **#366** (merged 2026-08-22) | **Space sync frames now carry an authorization rule, not just a signature check.** A directed sync frame (`sync-peer-map`, `sync-members`, `sync-messages`) is honoured only when its signing key is one the space has already bound to an identity — the registered owner key, or the registered signing key of a current, non-kicked member. Requiring the OWNER key alone would not have worked and is worth recording: only the owner-broadcast path signs with it, while every peer-to-peer sync signs with the sender's own per-space key, so an owner-only rule would have silently refused all peer sync. A roster-bootstrap exception keeps a fresh join working and closes permanently once any other member row arrives. Verified: new live-relay harness scenario including a delivery arm proving peer sync still works, mutation-proven red-on-revert, plus 1680 desktop tests. Two residuals recorded privately rather than left implicit. Desktop-only — the sibling client no longer participates in this protocol in either direction. |
| **OPEN** | **PRIVATE:** desktop `.agents/issues/.secret/` (`2026-08-20-sender-authentication-is-opt-in-mechanism-level-fix-design.md`) | **The mechanism-level design this recap called for at lines 108-115, now written up.** Diagnoses why per-type hardening keeps leaving gaps (the type list is opt-IN, so a new feature inherits nothing and nothing fails), and proposes compiler-enforced classification in `quorum-shared` plus one structural correction that is a prerequisite for the row above. 7 slices, each with its own rollout-safety note. **Needs a named owner for slices 3-7** — see §7 of that file, which argues this proposal has now been made twice and shelved once. Execution queue for this work is the private epic in the row below. |
| **DONE** | quorum-shared **#87** + quorum-desktop **#359**, then desktop **#361** (merged 2026-08-21) | Closes the "further control-message type" OPEN row above, plus two follow-on defects found while doing it. The type is now authorized against the verified signer on both storage paths, its ownership is pinned to that signer rather than read off the wire, and the send-side signing decision derives from the same list receivers enforce. #361 then bounded what one of its actions may reach, after an independent review of the #359 fix found a further, higher-severity defect in adjacent code. Verified: 1649 desktop tests, three live-relay scenarios each confirmed to fail against the prior behaviour. Detail kept OUT of this public repo. |
| **OPEN** | **PRIVATE:** desktop `.agents/issues/.secret/receive-auth-hardening/` | **The execution queue for the durability work**, created 2026-08-21: one brief per PR, eight of them, with an explicit order and per-brief definitions of done. Exists because this effort had been described twice and executed zero times, and "slices 3-7 of a design document" is not something any index or progress view can report as outstanding. It also records a SECOND failure mode the design above does not cover — a handler that is correctly enrolled and correctly verified, but authorizes a different object than the one it acts on. **Needs a named owner.** Detail kept OUT of this public repo; ask the operator. |
