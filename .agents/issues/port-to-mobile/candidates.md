...
- **#39 · Bookmarks are write-only on mobile — the panel exists and cannot be opened** — 📋 **PARTIAL** `[CV]` `SMALL` (found 2026-08-05)
  MEASURED by grep, not inferred: `setBookmarksPanelVisible(true)` appears **nowhere** in the mobile repo. The state is initialised `false` (`DMChatArea.tsx:107`, `SpaceChatArea.tsx:152`) and the only other write is the panel's own `onClose`, so `BookmarksPanel` — a complete, finished 252-line component with an empty state that reads "Long press a message and tap Bookmark to save it here" — never renders.
  Everything else is wired: the action sheet offers Bookmark / Remove Bookmark (`MessageActionSheet.tsx:282-285`), `useBookmarks` (`hooks/useUserConfig.ts:256`) adds and removes, and `services/config/configService.ts` stores them in MMKV and syncs them in the config blob. So a mobile user can bookmark a message and it will sync to desktop, but **cannot see their own bookmarks anywhere on mobile.**
  Fix is a header entry point, not a feature: the panel takes `visible` / `onClose` / `bookmarks` / `onRemoveBookmark` / `onNavigateToBookmark` and both call sites already pass all five. Desktop's precedent is a chat-header icon rendered only when the current context has at least one bookmark, plus a NavRail item for the global list.
  ⚠️ Do **not** read the desktop `.agents` bookmarks doc as describing mobile — it documents the desktop surfaces only. Related: bookmark payload bloat, [2026-08-05-bookmarks-are-75-percent-of-the-config-blob.md](../2026-08-05-bookmarks-are-75-percent-of-the-config-blob.md), which needs a mobile-side strip regardless of whether this panel is ever opened.
