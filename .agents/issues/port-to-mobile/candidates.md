...
- **#10 · "New messages" separator + jump-to-first-unread** — 📋 ABSENT `[FP]` `MED`
  UI/scroll only, no shared work. Pairs with #28. *(Adjacent, not the same: mobile `.agents/tasks/.todo/2026-06-18-channel-unread-dot-lastread-timestamp.md` is the channel-LIST unread dot, a different `lastReadTimestamp` consumer — reuse its plumbing, don't treat it as this drop.)*
...
- **#39 · Bookmarks are write-only on mobile — the panel exists and cannot be opened** — 📋 **PARTIAL** `[CV]` `SMALL` (found 2026-08-05)

  **No task file exists for this in either repo** (checked mobile `.agents/issues/` and `quorum-shared-migration/mobile-tasks-pending.md` on 2026-08-05 — the single mobile hit for "bookmark" is a light-theme contrast audit that lists `BookmarksPanel.tsx` as a file containing muted text). This entry is the whole record; drop a real task file in the mobile repo when it is picked up.

  **The gap.** MEASURED by grep, not inferred: `setBookmarksPanelVisible(true)` appears **nowhere** in the mobile repo. The state is initialised `false` (`DMChatArea.tsx:107`, `SpaceChatArea.tsx:152`) and the only other write is the panel's own `onClose`, so `BookmarksPanel` — a complete, finished 252-line component with an empty state reading "Long press a message and tap Bookmark to save it here" — never renders. Everything else is wired: the action sheet offers Bookmark / Remove Bookmark (`MessageActionSheet.tsx:282-285`), `useBookmarks` (`hooks/useUserConfig.ts:256`) adds and removes, and `services/config/configService.ts` stores them in MMKV and syncs them in the config blob. So a mobile user can bookmark a message and it syncs to desktop, but **cannot see their own bookmarks anywhere on mobile**. The panel takes `visible` / `onClose` / `bookmarks` / `onRemoveBookmark` / `onNavigateToBookmark` and both call sites already pass all five, so the missing piece is an entry point, not a feature. Desktop's precedent: a chat-header icon rendered only when the current context has at least one bookmark, plus a rail item for the global list.

  **Ships with it — the payload strip** (from [2026-08-05-bookmarks-are-75-percent-of-the-config-blob.md](../2026-08-05-bookmarks-are-75-percent-of-the-config-blob.md)). Desktop shipped a fix for bookmarks being 69% of the encrypted config blob; mobile publishes the same blob and has no strip. Mobile never wrote `senderIcon` itself, but bookmarks it adopted from desktop before that fix still carry one, so it can re-inflate the blob for the whole account. Two lines: apply `stripBookmarkSenderIcons` inside `getLocalBookmarks` (`services/config/configService.ts:104`) and `saveLocalBookmarks` (`:115`) — between them the only read and write points for MMKV bookmark storage, so that covers the upload (`:599`), the stored config (`:465`) and the remote merge (`:410`) at once.

  ⚠️ **Blocked on a shared PUBLISH, not a merge.** `stripBookmarkSenderIcons` landed in shared `master` (PR #75) and the version is bumped to **`2.1.0-40`**, but the shared repo has **no CI publish workflow** — someone must run `npm publish` by hand. npm's latest is still `2.1.0-39`, which is what mobile pins. Until `-40` is on npm, mobile cannot import the helper. Check with `npm view @quilibrium/quorum-shared versions --json` before starting, then bump mobile's pin.

  🔴 **If you add avatars to the mobile panel, read this first — desktop shipped this bug and caught it in review.** Bookmarks no longer store the sender's avatar; it is resolved at render from `cachedPreview.senderAddress`. **A DM conversation record is keyed by the COUNTERPART and carries the COUNTERPART's identity**, so `conversation.icon` is only usable when that counterpart IS the sender. Bookmarking your own message in a DM is ordinary, and reading that field blindly renders your name beside the other person's face; a group conversation's icon belongs to no member at all. The space member row is read by `(spaceId, senderAddress)` and needs no such check. Desktop's ladder, worth mirroring: per-space override → roster global slot → conversation (gated on counterpart) → own avatar → public profile → coloured initials. See `src/hooks/business/bookmarks/useBookmarkSenderIcon.ts` and its tests. Mobile's panel renders no avatar today, so this is a trap for whoever adds one, not a current defect.

  ⚠️ Do **not** read the desktop `.agents` bookmarks doc as describing mobile — it documents the desktop surfaces only.
...
- [`.agents/issues/.done/reply-notification-system.md`](../.done/reply-notification-system.md) — completed architecture task doc
...
- [`src/hooks/business/mentions/useMentionNotificationSettings.ts`](../../../src/hooks/business/mentions/useMentionNotificationSettings.ts) — event-type filter persistence (has its own sync bug — see [`2026-06-07-mention-type-filter-not-synced.md`](../.done/2026-06-07-mention-type-filter-not-synced.md))
...
- [`.agents/issues/.done/2026-06-07-align-notification-settings-with-mobile.md`](../.done/2026-06-07-align-notification-settings-with-mobile.md) — desktop-side UX rename that left this convergence work deferred to this entry
