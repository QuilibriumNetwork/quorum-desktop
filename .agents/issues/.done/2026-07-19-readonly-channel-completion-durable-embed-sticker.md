...
closes-bug: .agents/issues/.done/2026-06-12-readonly-channel-receive-side-enforcement-gaps.md
...
- **The bug this completes:** `.agents/issues/.done/2026-06-12-readonly-channel-receive-side-enforcement-gaps.md` (the two gaps = durable + embed/sticker).
...
- **Shared primitives (published, consume as-is):** `@quilibrium/quorum-shared@2.1.0-35` — `buildMessageFingerprint`, `resolveVerifiedSender` (`quorum-shared/src/utils/messageAuth.ts`), `canManageReadOnlyChannel` (`quorum-shared/src/utils/channelPermissions.ts`).
- **Private tracking issue (the whole effort):** https://github.com/QuilibriumNetwork/quorum-app-prod/issues/1.
- **Mobile side:** the full mobile audit + reference list is in `quorum-mobile/.agents/tasks/2026-06-25-space-control-msg-auth-signature-design.md` (gitignored/local) — mirror it for the mobile half of this task.
- **Cross-repo rules:** `quorum-atlas.md` (esp. iOS-review-only, ship/PR conventions).
