---
type: task
title: "DM Read Receipts Design — Phase 2"
status: done
created: 2026-03-22
---
...
**Phase 1 Spec:** `.agents/issues/.done/2026-03-18-dm-delivery-receipts-design.md`
...
| Ack persistence vs display | Settings gate persistence, not display | Incoming read acks are only persisted when the user's `readReceipts` setting is ON. Once persisted, checkmarks are always visible even if the setting is later turned OFF. Past receipts remain; only future ack processing is blocked. |
...
- Display is unconditional: if `readAt` exists → ✓✓; if `deliveredAt` exists → ✓. Settings gate *persistence*, not display. Since `readAt` is only written when the user's `readReceipts` setting is ON, toggling OFF prevents new ✓✓ from appearing without retroactively hiding existing ones.
...
- OFF = no read acks leave the device, incoming read acks are dropped (not persisted)
- `useReadReceipt` hook checks the setting before observing — if off, no observers mounted
- Already-persisted ✓✓ from when the setting was ON remain visible after toggling OFF
- Reciprocal sending: their setting gates sending acks, your setting gates persisting incoming acks
