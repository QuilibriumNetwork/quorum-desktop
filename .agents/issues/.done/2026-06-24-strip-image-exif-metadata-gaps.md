...
> **✅ DONE 2026-06-24** (branch `fix/strip-exif-on-image-upload`). All three gaps addressed in `src/utils/imageProcessing/compressor.ts`:
> - **Gap 1** — JPEG/TIFF (where EXIF/GPS lives) now excluded from the skip-compression path, so small geotagged photos always get re-encoded. Perf shortcut kept for PNG/WebP (no GPS EXIF).
> - **Gap 2** — `strict: false` added so compressorjs always returns the stripped re-encode instead of handing back the raw original when it grows.
> - **Gap 3** — GIF passthrough kept and documented in-code as an accepted non-gap (no standardized GPS EXIF in GIF; re-encoding would flatten animation).
> Chose the surgical JPEG/TIFF-only carve-out over dropping `skipCompressionThreshold` to 0, to preserve the perf shortcut for the common small-PNG-icon case. Typecheck, lint, full build green. Acceptance item "verify with exiftool on real uploads" left for dev-env confirmation.
