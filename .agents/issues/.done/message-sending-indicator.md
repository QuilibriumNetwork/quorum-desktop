...
When the persistent background action queue is implemented (see `.agents/issues/.archived/background-action-queue-with-worker-crypto.md`), this feature can be upgraded for true offline support:
...
- GitHub Issue: https://github.com/QuilibriumNetwork/quorum-desktop/issues/51
- Background Action Queue Task: `.agents/issues/.archived/background-action-queue-with-worker-crypto.md`
