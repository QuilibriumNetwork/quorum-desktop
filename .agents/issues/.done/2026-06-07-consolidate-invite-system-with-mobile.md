...
- `.agents/issues/.done/2025-09-22-public-invite-link-intermittent-expiration.md` annotated as `likely-resolved-by-consolidation` (original "first 1-2 joiners succeed" symptom matches the old eval-pop-per-join behavior that the new server-side model removes).
- `.agents/issues/.open/2026-01-09-delete-public-invite-link.md` annotated with the consolidation context — task is still valid, narrower scope now.
