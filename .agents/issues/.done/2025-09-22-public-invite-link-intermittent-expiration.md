...
>
> Mobile's join code at [`quorum-mobile/hooks/chat/useSpaceActions.ts:271-279`](../../../../quorum-mobile/hooks/chat/useSpaceActions.ts) handles this by using the eval's OWN ephemeral pubkey when the server provides it, falling back to the manifest's only on legacy servers. Desktop now does the same — see [task `2026-06-08-fix-join-invite-link.md`](2026-06-08-fix-join-invite-link.md) for the full diagnosis and fix.
>
